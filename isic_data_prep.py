...
print(f"  Saved: merged_cleaned.csv  ({df.shape[0]:,} rows x {df.shape[1]} columns)")
...
df_malignant.to_csv('datasetv1.csv', index=False)
df_benign.to_csv('datasetv2.csv',    index=False)

print(f"  datasetv1.csv — Malignant only : {len(df_malignant):,} rows")
print(f"  datasetv2.csv — Benign only    : {len(df_benign):,} rows")
...
pd.DataFrame(range_rows).to_csv('feature_ranges.csv', index=False)
print(f"\n  Saved: feature_ranges.csv")
...
output_name = f'merged_cleaned_binned_{RANGE_VERSION}.csv'
...
df_v3.to_csv('datasetv3.csv', index=False)
print(f"  Saved: datasetv3.csv  ({df_v3.shape[0]:,} rows x {df_v3.shape[1]} columns)")
